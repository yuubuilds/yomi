...
Yomi は、テキストファイルを落ち着いて読むための軽量デスクトップアプリです。

ログ、メモ、Markdown、コード片、文字コードが混在した古いファイルなどを、素早く開いて確認するために作りました。複数ファイルをタブで開き、文字コードを自動判定し、シンタックスハイライト、空白文字の可視化、改行コードの確認・変換を必要に応じて切り替えられます。

CodeMirror 6 をベースにした読みやすいエディタ表示と、Neutralinojs による軽量なデスクトップ実行環境を組み合わせ、余計な機能を増やしすぎず「開く、読む、確認する、必要なら少し直す」ことに集中できるツールを目指しています。
